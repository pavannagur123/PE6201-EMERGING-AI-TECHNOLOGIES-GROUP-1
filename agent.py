...
def run_case(case_id, problem=None, approve=None, verbose=False):
    ...
    transcript = [{"role": "user",
                   "content": ("Process claim %s. Use tools to establish "
                               "the routing-table outcome and return only a "
                               "structured decision record." % case_id)}]
    # The initial task must be visible to a live model. The scripted backend
    # intentionally ignores the transcript, but a live backend otherwise has
    # no way to know which claim it was asked to process.
